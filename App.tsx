...
import { BOOKS, NEWS, BOOK_INTRODUCTIONS } from './data/mockData';
import { User, Book, BookIntroduction, NewsItem } from './types';

type ViewState = 'home' | 'intro-list' | 'intro-detail' | 'news-list' | 'news-detail' | 'doc-overview' | 'doc-category';
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 selection:bg-emerald-100 flex flex-col font-['Inter']">
      ...
      <main className="flex-1">
        {currentView === 'home' && (
          <>
            ...
            <div className="max-w-[1440px] mx-auto px-6 space-y-12 pb-16">
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <section className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {BOOK_INTRODUCTIONS.slice(0, 4).map(intro => (
                      <div key={intro.id} className="group cursor-pointer" onClick={() => navigateToIntroDetail(intro)}>
                        ...
                        <h3 className="text-[12px] font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-[#00a651] transition-colors h-9 mb-1">{intro.title}</h3>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <div className="space-y-6">
                    {NEWS.slice(0, 4).map(news => (
                      <div key={news.id} className="flex gap-5 group cursor-pointer items-start" onClick={() => navigateToNewsDetail(news)}>
                        ...
                        <div className="flex-1">
                          <h3 className="text-[13px] font-bold text-slate-800 group-hover:text-[#00a651] line-clamp-1 transition-colors">{news.title}</h3>
                          <p className="text-[11px] text-slate-500 line-clamp-2 italic mt-1">{news.summary}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Hiển thị tất cả danh mục theo yêu cầu */}
              <BookSection title="Sách điện tử" books={getBooksByCategory('Sách điện tử').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Sách nói" books={getBooksByCategory('Sách nói').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Bài giảng điện tử" books={getBooksByCategory('Bài giảng điện tử').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Video" books={getBooksByCategory('Video').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Album ảnh" books={getBooksByCategory('Album ảnh').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Kỹ năng sống" books={getBooksByCategory('Kỹ năng sống').slice(0, 5)} onBookClick={handleBookClick} />
              <BookSection title="Báo, tạp chí" books={getBooksByCategory('Báo, tạp chí').slice(0, 5)} onBookClick={handleBookClick} />
            </div>
          </>
        )}

        {currentView === 'doc-overview' && (
          <DocumentOverview 
            books={BOOKS} 
        ...
        {currentView === 'doc-category' && (
          <CategoryDetailView 
            category={selectedCategory} 
            books={BOOKS} 
            onBookClick={handleBookClick} 
            onNavigateHome={navigateToHome} 
            onNavigateOverview={navigateToDocOverview} 
          />
        )}

        {currentView === 'intro-list' && (
          <IntroList introductions={BOOK_INTRODUCTIONS} onSelectIntro={navigateToIntroDetail} onBack={navigateToHome} />
        )}
        {currentView === 'intro-detail' && selectedIntro && (
          <IntroDetail intro={selectedIntro} allIntros={BOOK_INTRODUCTIONS} onNavigateDetail={navigateToIntroDetail} onNavigateList={navigateToIntroList} onNavigateHome={navigateToHome} />
        )}
        {currentView === 'news-list' && (
          <NewsList newsList={NEWS} onSelectNews={navigateToNewsDetail} onBack={navigateToHome} />
        )}
        {currentView === 'news-detail' && selectedNews && (
          <NewsDetail news={selectedNews} allNews={NEWS} onNavigateDetail={navigateToNewsDetail} onNavigateList={navigateToNewsList} onNavigateHome={navigateToHome} />
        )}
      </main>
