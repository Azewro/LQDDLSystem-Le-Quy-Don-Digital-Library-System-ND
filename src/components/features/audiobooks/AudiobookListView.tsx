...
import { Headphones, Eye, Heart, FolderOpen, ChevronRight, LayoutGrid, Search } from 'lucide-react';
import { Audiobook, AudiobookFolder } from '@/types';

interface AudiobookListViewProps {
    audiobooks: Audiobook[];
    folders: AudiobookFolder[];
    onBookClick: (book: Audiobook) => void;
    onNavigateHome: () => void;
    onNavigateOverview: () => void;
}

const AudiobookListView: React.FC<AudiobookListViewProps> = ({
    audiobooks,
    folders,
    onBookClick,
    onNavigateHome,
    ...
    const folderTree = useMemo(() => {
        ...
        const getChildren = (parentId: string): AudiobookFolder[] =>
    ...
    const folderBookCount = useMemo(() => {
        ...
        folders.forEach(f => {
            ...
            counts[f.id] = audiobooks.filter(b => b.folder_id && folderIds.includes(b.folder_id)).length;
        });
        return counts;
    }, [audiobooks, folders]);
    ...
    const filteredBooks = useMemo(() => {
        ...
        if (selectedGrade !== 'Tất cả') {
            ...
            result = result.filter(b => b.grade.includes(gradeNum) || b.grade === 'Tất cả');
        }
        ...
        return result;
    }, [audiobooks, searchTerm, selectedGrade, selectedFolder]);
    ...
    const renderFolderItem = (folder: AudiobookFolder, depth: number = 0) => {
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-6 py-10">
            ...
            <nav className="flex items-center gap-2 text-[12px] font-medium text-slate-400 mb-8 uppercase tracking-widest">
                ...
                <span className="opacity-30">/</span>
                <span className="text-emerald-600">Sách nói</span>
            </nav>
                ...
                <aside className="w-72 flex-shrink-0">
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sticky top-28">
                        ...
                        <div className="mb-8">
                            ...
                            <div className="space-y-1 max-h-[400px] overflow-y-auto custom-scrollbar text-slate-600">
                        ...
                        <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-lg font-black text-slate-800">{audiobooks.length}</p>
                                    <p className="text-[10px] font-bold text-slate-500 uppercase">Sách nói</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                {filteredBooks.map((book) => (
                                    <div key={book.id} className="group cursor-pointer" onClick={() => onBookClick(book)}>
                                        <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-slate-100 mb-3 shadow-sm group-hover:shadow-xl group-hover:-translate-y-1 transition-all bg-slate-100">
                                            <img
                                                src={book.cover_url || '/images/default-audio-cover.png'}
                                            ...
                                            <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                                                <button className="w-full py-2 bg-[#00a651] text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2">
                                                    <Headphones className="w-3.5 h-3.5" /> Nghe sách
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 text-slate-400 text-[10px] font-bold">
                                            <span className="flex items-center gap-1.5">
                                                <Eye className="w-3.5 h-3.5" /> {book.views} lượt nghe
                                            </span>
                                            <span className="flex items-center gap-1.5">
...
export default AudiobookListView;
