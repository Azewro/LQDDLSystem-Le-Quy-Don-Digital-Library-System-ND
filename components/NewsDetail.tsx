      ...
      <div className="grid lg:grid-cols-12 gap-12">
        <article className="lg:col-span-8 bg-white p-10 rounded-2xl border border-slate-100 shadow-sm">
          <header className="mb-10 border-b border-slate-50 pb-8">
            <h1 className="text-3xl font-black text-slate-800 leading-tight mb-6 uppercase">
              {news.title}
            </h1>
            <div className="flex items-center gap-6 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
              <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-emerald-500" /> {news.date}</span>
              <span className="flex items-center gap-1.5"><User className="w-4 h-4 text-emerald-500" /> {news.author}</span>
              <span className="flex items-center gap-1.5"><Eye className="w-4 h-4 text-emerald-500" /> {news.views} lượt xem</span>
            </div>
          </header>

          <div 
            className="prose prose-slate max-w-none text-slate-600 text-lg leading-relaxed space-y-6"
            dangerouslySetInnerHTML={{ __html: news.content || '<p>Nội dung đang được cập nhật...</p>' }}
          />
              ...
              {allNews.filter(n => n.id !== news.id).slice(0, 3).map(item => (
                <div key={item.id} className="flex gap-6 group cursor-pointer items-start" onClick={() => onNavigateDetail(item)}>
                  ...
                  <div className="flex-1 py-1">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-[10px] font-bold uppercase text-slate-400">
                       <span className="text-emerald-500">{item.date}</span>
                    </div>
                  </div>
                </div>
              ))}
