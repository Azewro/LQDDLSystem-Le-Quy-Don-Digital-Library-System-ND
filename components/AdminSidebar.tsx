...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ activeMenu, isCollapsed, onMenuClick }) => {
  ...
  return (
    <aside 
      className={`bg-slate-900 border-r border-slate-800 flex flex-col h-full sticky top-20 transition-all duration-300 ease-in-out ${
        ...
        <nav className="space-y-2">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center px-4 py-3.5 rounded-2xl transition-all group relative ${
                activeMenu === item.id 
          ...
          <div className="space-y-1">
            <button
              ...
              className={`w-full flex items-center px-4 py-3.5 rounded-2xl transition-all group relative ${
                activeMenu.startsWith('news') || activeMenu.startsWith('intro')
                ? 'bg-[#00a651] text-white shadow-lg shadow-emerald-500/20' 
                : 'text-slate-400 hover:bg-white/5 hover:text-white'
              } ${isCollapsed ? 'justify-center px-0' : 'justify-between'}`}
            >
