        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 cursor-pointer" onClick={onNavigateHome}>
            ...
            <div className="flex flex-col">
              <h1 className="text-white font-black text-[14px] leading-tight uppercase tracking-tight">HỆ THỐNG QUẢN TRỊ</h1>
        ...
        <div className="flex items-center gap-6">
          ...
          {user && (
            <div className="flex items-center gap-4">
              <button onClick={onProfileClick} className="flex items-center gap-3 px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-2xl hover:bg-slate-750 transition-all group">
                 <div className="w-7 h-7 bg-emerald-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 group-hover:rotate-12 transition-transform">
                    <UserIcon className="w-4 h-4" />
                 </div>
                 <div className="text-left hidden lg:block">
                   <p className="text-[11px] font-black text-slate-200 uppercase leading-none">{user.name.split(' ').pop()}</p>
                   <p className="text-[9px] font-bold text-slate-500 uppercase mt-1">Admin</p>
                 </div>
              </button>
              <button onClick={onLogout} className="text-slate-500 hover:text-red-400 transition-colors p-2"><LogOut className="w-5 h-5" /></button>
            </div>
          )}
        </div>
      </div>
