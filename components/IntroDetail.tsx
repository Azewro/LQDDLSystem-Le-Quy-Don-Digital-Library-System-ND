...
import { Calendar, User, Eye, ArrowLeft, ChevronRight, Share2, Printer } from 'lucide-react';
      ...
      <div className="grid lg:grid-cols-12 gap-12">
        <article className="lg:col-span-8 bg-white p-6 md:p-12 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <header className="mb-10 border-b border-slate-50 pb-10">
            ...
            <div className="flex flex-wrap items-center justify-between gap-6">
              <div className="flex items-center gap-6 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-emerald-500" /> {intro.date}</span>
                <span className="flex items-center gap-2"><User className="w-4 h-4 text-emerald-500" /> {intro.author}</span>
                <span className="flex items-center gap-2"><Eye className="w-4 h-4 text-emerald-500" /> {intro.views} lượt xem</span>
              </div>
          ...
          <section className="mt-20 pt-10 border-t border-slate-100">
            <h3 className="text-slate-800 font-black text-lg border-l-4 border-[#00a651] pl-4 uppercase tracking-tight mb-8">Giới thiệu sách khác</h3>
            <div className="space-y-6">
              {allIntros.filter(i => i.id !== intro.id).slice(0, 3).map(item => (
                <div key={item.id} className="flex gap-6 group cursor-pointer items-start" onClick={() => onNavigateDetail(item)}>
                  <div className="w-40 h-28 rounded-xl overflow-hidden shadow-sm flex-shrink-0 border border-slate-50">
            ...
            <div className="space-y-6">
              {allIntros.sort((a,b) => b.views - a.views).slice(0, 5).map((item) => (
                <div key={item.id} className="flex gap-4 group cursor-pointer" onClick={() => onNavigateDetail(item)}>
                  <div className="w-20 h-24 rounded-lg overflow-hidden flex-shrink-0 shadow-sm border border-slate-50">
                    <img src={item.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform" alt={item.title} />
                  </div>
                  <div>
                    <h4 className="text-[12px] font-bold text-slate-800 line-clamp-2 group-hover:text-[#00a651] mb-1 leading-snug">{item.title}</h4>
                    <span className="flex items-center gap-1.5 text-[9px] text-slate-400 font-bold uppercase tracking-widest"><Eye className="w-3.5 h-3.5 text-emerald-500" /> {item.views} lượt xem</span>
                  </div>
                </div>
              ))}
            </div>
